...
pub fn TimerDisplay(props: &Props) -> Html {
    ...
    let increase_session_length = {
        ...
        // this will increase the session by 5mins
        Callback::from(move |_: ()| session_length.set(*session_length + 60 * 5))
    };

    let decrease_session_length = {
        let session_length = props.session_length.clone();

        // this will decrease the session by 5mins
        Callback::from(move |_: ()| session_length.set(*session_length - 60 * 5))
    };
    ...
    html! {
        <div class={classes!("flex", "flex-col", "space-y-2", "items-center")}>
            <div class={classes!("flex", "flex-row", "space-x-4")}>
                <button
                    onclick={move |_| {decrease_session_length.emit(())}}
                    class={classes!("bg-red-500")}>
                    {"- 5"}
                </button>
                    <p class={classes!("text-5xl")}>{get_session_display}</p>
                    <button
                    onclick={move |_| {increase_session_length.emit(())}}
                    class={classes!("bg-green-500")}>
                    {"+ 5"}
                </button>
            </div>
